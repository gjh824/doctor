<template>
  <div>
    <div>
      <div class="HeaNav">
        <router-link to="">出院管理</router-link>
      </div>
    </div>
    <div class="elecen">
      <div class="bing">
        <div class="change">
          <form action="" class="g_iH_form">
            ...
            <div class="g_inp g_inp_cen">
              <input type="date" name="" placeholder="选择出院时间" />
            </div>
            <div class="g_inp">
              <input type="text" name="" placeholder="选择入房病号" />
            </div>
        ...
        <div>
          <table class="g_hs_tab">
            ...
            <tr class="g_tr" v-for="(item, k) in msg" :key="k">
              <td><input type="checkbox" class="checkbox" disabled @click="EMRoneChoose(k,item.out_id)" /></td>
              <td>{{ item.zyh }}</td>
              <td>{{ item.xm }}</td>
              <td>{{ item.xb }}</td>
              <td>{{ item.nl }}</td>
              ...
              <td>{{ item.cysj }}</td>
              <td>
                <router-link to="/dayin" class="g_tab_a"
                  ><span class="fa fa-pencil"></span
                ></router-link>
                <router-link to="" class="g_tab_a"
                  ><span class="fa fa-trash" @click="dele(k,item.out_id)"></span
                ></router-link>
              </td>
            </tr>
          </table>
...
export default {
  ...
  data() {
    return {
        ...
        infosa:{
            pag:'',//{'page':i+1}
            url:'http://localhost:3000/doctor/outHosPag'
        }
    };
  },
  methods: {
    dele(k,id) {
      alert("删除下标" + k);
    //   this.tab_info.splice(k, 1);
      this.msg.splice(k,1)
        this.$axios.get(this.HOST+'/ouToneDele',{params:{id:id}})
        .then(response=>{
    ...
    // 初始 获得 病人 数据
    getDzbl(){
        this.$axios.get(this.HOST+'/chuyuan')
        .then(response=>{
        ...
        this.$axios.get(this.HOST+'/outHosPag',{params:{page:0}})
        .then(result=>{
    ...
    EMRmanyDele(){
        ...
        if(this.falg){
            ...
            // 发送 对应序号 给后端
            this.$axios.get(this.HOST+'/outHosdele',{params:{id:this.id}})
            .then(response=>{
                console.log(response)
            })
            .catch(error=>{
