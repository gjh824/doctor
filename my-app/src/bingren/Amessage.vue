            ...
            <div class="bing" >
                ...
                <table class="g_hs_tab">
                    <tr class="g_tab_tou">
                        ...
                        <th>问题描述</th>
                        <th>主要症状</th>
                        <th class="g_Rd_th">所用药物</th>
                        <!-- <th>操作</th> -->
                    </tr>
                    <tr class="g_tr" v-for="(item,k) in msg" :key="k">
                        <td><input type="checkbox" class="checkbox" disabled @click="EMRoneChoose(k,item.att_id)"></td>
                        <td>{{item.xh}}</td>
                        <td>{{item.xm}}</td>
                        <td>{{item.xb}}</td>
                        <td>{{item.nl}}</td>
                        ...
                        <td>{{item.wtms}}</td>
                        <td>{{item.zyzz}}</td>
                        <td class="g_Rd_td">{{item.syyw}}</td>
                        <td><router-link to="" class="g_tab_a"><span @click="answer(item.att_id,k)">解答问题</span></router-link></td>
                        <!-- <td><router-link to="" class="g_tab_a"><span>查看详情</span></router-link></td> -->
                    </tr>
                </table>
                <div class="change">
                    <div class="g_dele">
               ...
               <div class="An_div">
                    <form action="" class="g_Aa_mb_form" @submit.prevent>
                        ...
                        <div class="g_Aa_mb_form_di">
                            <p>解答: </p>
                            <textarea v-model="amend" name="" id="An_text" cols="30" rows="5"></textarea>
...
export default {
    ...
    data() {
        return {
            ...
            infosa:{
                pag:'',//{'page':i+1}
                url:'http://localhost:3000/doctor/attPag'
            }
        }
    },
    methods: {
        answer(id,k){
            this.k_info=k
            this.att_id=id
            let An_div=document.querySelector('.An_div')
            An_div.style.display='block'
        },
        close(){
            let An_div=document.querySelector('.An_div')
            An_div.style.display='none'
        },
        sour(){
            let An_div=document.querySelector('.An_div')
            An_div.style.display='none'
            this.$axios.post(this.HOST+'/attUpData',{amend:this.amend,id:this.att_id})
            .then(response=>{
                // console.log(response.data)
                this.msg[this.k_info].syyw=response.data
            })
            .catch(error=>{
                console.log(error)
            })
        ...
        // 初始 获得 病人 数据
        getDzbl(){
            this.$axios.get(this.HOST+'/amessage')
            .then(response=>{
            ...
            this.$axios.get(this.HOST+'/attPag',{params:{page:0}})
            .then(result=>{
        ...
        EMRmanyDele(){
            ...
            if(this.falg){
                ...
                // 发送 对应序号 给后端
                this.$axios.get(this.HOST+'/attdele',{params:{id:this.id}})
                .then(response=>{
